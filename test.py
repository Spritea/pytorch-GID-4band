...
def test(args,cfg):
    ...
    for j in tqdm(range(len(IMG_Str))):
        ...
        #PIL outputs R-G-B
        #my_loader.py must make opencv transform input img to R-G-B
        #for 4 band, the model is trained with B-G-R
        #so test.py must use B-G-R input img
        #using opencv is wrong,should use pil
        #the line below not run yet
        ...
        a=outputs.data.max(1)
        pred = np.squeeze(outputs.data.max(1)[1].cpu().numpy(), axis=0)

        decoded = loader.decode_segmap(pred)
        out_path="test_out/CAN_res50_4band_data07/"+Path(img_path).stem+".png"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        default="configs/mv3_1_true_2_res50_4band_GID_data07.yml",
